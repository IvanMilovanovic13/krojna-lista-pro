...
def _translate_instruction_line(line: str, lang: str = "sr") -> str:
    ...
    exact = {
        ...
        "-- KORAK 5 - ZAVRSNA PROVERA --": "-- STEP 5 - FINAL CHECK --",
        ...
        "-- KORAK 3 - MONTAŽA NA ZID --": "-- STEP 3 - WALL INSTALLATION --",
...
def _polish_instruction_line_en(line: str) -> str:
    ...
    # Prevod preostalih srpskih recenica koje nisu uhvacene u _translate_instruction_line.sentences
    # VAZNO: destination mora biti ENGLESKI (bio bug: sve je bilo prevedeno na PORTUGALSKI)
    sr_replacements = [
        ("Umetni dno izmedju bocnih stranica i spoji ga prema busenjima.", "Insert the bottom panel between the side panels and join it according to the drill points."),
        ("Umetni dno i gornju plocu izmedju bocnih stranica i spoji ih prema busenjima.", "Insert the bottom and top panel between the side panels and join them according to the drill points."),
        ("Zatim postavi gornju plocu i proveri da korpus ostane ravan.", "Then position the top panel and check that the carcass remains square."),
        ("Uzmi levu i desnu bocnu stranicu i postavi ih uspravno tako da prednje ivice gledaju ka napred.", "Take the left and right side panel and place them upright so the front edges face forward."),
        ("Pre sledeceg koraka proveri da korpus stoji ravno i da nigde nije uvijen.", "Before the next step, check that the carcass is straight and not twisted anywhere."),
        ("Montiraj šarke, okači vrata i podesi male razmake tako da se vrata lako otvaraju i ne zapinju.", "Fit the hinges, hang the doors and adjust the gaps so the doors open easily without catching."),
        ("Obeleži tačnu visinu na zidu, montiraj zidni nosač ili šinu i tek onda okači element.", "Mark the exact height on the wall, install the wall bracket or rail, and only then hang the unit."),
        ("Pre završnog stezanja proveri libelom da je element potpuno ravan levo-desno i napred-nazad.", "Before final tightening, use a level to check that the unit is perfectly level left-right and front-back."),
        ("Tek kada je element ravan i čvrsto vezan, nastavi na sledeći zidni element ili frente.", "Only when the unit is level and firmly secured, continue to the next wall unit or fronts."),
        ("Umetni ledjnu plocu na zadnju stranu elementa.", "Insert the back panel into the rear of the unit."),
        ("Pre konacnog stezanja proveri dijagonale da element bude pod pravim uglom.", "Before final tightening, check the diagonals to ensure the unit is at right angles."),
        ("Prvo proveri da li rerna slobodno ulazi u otvor korpusa i da li ima prostora za ventilaciju.", "First check that the oven slides freely into the carcass opening and that there is room for ventilation."),
        ("Zatim isezi otvor u radnoj ploci prema šablonu ploce za kuvanje.", "Then cut the opening in the worktop according to the hob manufacturer's template."),
        ("Zatim isezi otvor u radnoj ploci prema sablonu ploce za kuvanje.", "Then cut the opening in the worktop according to the hob manufacturer's template."),
        ("Lazni front ili front fioke montiraj tek nakon probe zazora oko rerne i vrata rerne.", "Fit the dummy front or drawer front only after checking the clearance around the oven and oven door."),
        ("Montiraj noge ili stopice, zatim nivelisi element na mestu ugradnje.", "Fit the legs or feet, then level the unit at the installation point."),
        ("Tek kada je korpus ravan i stabilan montiraj radnu plocu ili nastavi na sledeci element.", "Only when the carcass is level and stable, fit the worktop or continue to the next unit."),
        ("Izbusiti rupe za konfirmate i zleb za ledjnu plocu.", "Drill the confirmat holes and cut the groove for the back panel."),
        ("Ugradna rerna", "Built-in oven"),
        ("Ploca za kuvanje", "Hob"),
        ("Fioka ispod rerne", "Drawer below oven"),
        ("#3 - Rerna+ploča", "#3 - Oven + hob"),
        ("Rerna+ploča", "Oven + hob"),
        ("Rerna+ploca", "Oven + hob"),
        ("KORAK 4 - VRATA", "STEP 4 - DOORS"),
        ("KORAK 3 - MONTAŽA NA ZID", "STEP 3 - WALL INSTALLATION"),
        # Popravka lose kombinovanih recenica koje nastaju kao artefakt prevoda
        ("Drill the confirmat holes and recorte the groove for the back panel.", "Drill the confirmat holes and cut the groove for the back panel."),
        ("Fit the hinges, hang the portas and set 2 mm reveals.", "Fit the hinges, hang the doors and set 2 mm reveals."),
        ("Run water and confira whether the trap, tap and all connections leak.", "Run water and check whether the trap, tap and all connections leak."),
        # Preostali srpski fragmenti koji mogu proci kroz prethodni korak
        ("rernu", "the oven"),
        ("rerne", "the oven"),
        ("radnoj ploci", "the worktop"),
        ("radnu plocu", "the worktop"),
        ("radna ploca", "worktop"),
        ("ploce za kuvanje", "hob"),
        ("sablonu", "template"),
        ("šablonu", "template"),
        ("sledeci element", "the next unit"),
    ]
